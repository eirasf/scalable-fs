 ...
 * <pre>
 * -L &lt;num threads&gt;
 * Specify the maximum number of threads to be
 ...
 */
public class SVMThreadedAttributeEval 
  extends ASEvaluation
  implements AttributeEvaluator, 
             OptionHandler, 
             TechnicalInformationHandler {
	
	private class RankerBySVM extends Thread
	{
		private int[][] result;
		...
		public RankerBySVM(int minIndex, int numIndices, Instances data)
		{
			this.minIndex=minIndex;
			this.numIndices=numIndices;
			this.data=data;
			this.result=new int[numIndices][];
		}
		
		public void run()
		{
			for (int i=0; i<numIndices; i++)
				result[i]=rankBySVM(i+minIndex, data);
		}
  ...
  /** for serialization */
  static final long serialVersionUID = -6489995709033967447L;
   ...
   * Constructor
   */
  public SVMThreadedAttributeEval() {
    resetOptions();
  }
  ...
  public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(new Option("\tSpecify the maximum number of threads to be\n"
  ...
  public String[] getOptions() {
    ...
    options[current++] = "-L";
    options[current++] = "" + ((getN_threads()==Integer.MAX_VALUE)?0:getN_threads());
  ...
  public void buildEvaluator(Instances data) throws Exception {
	...
	// Get ranked attributes for each class separately, one-vs-all
	int[][] attScoresByClass;
	int numAttr = data.numAttributes() - 1;
	if (data.numClasses() > 2)
	{
		int numThreads = Runtime.getRuntime().availableProcessors();
		if (numThreads > n_threads) numThreads = n_threads;
		int numClasses=data.numClasses();
		if (numThreads > numClasses) numThreads = numClasses;
		
		RankerBySVM[] rankers=new RankerBySVM[numThreads];
		int nextClass=0;
		int classesPerThread=(int) Math.floor(numClasses/numThreads);
		for (int i = 0; i < numThreads; i++)
		{
			int classesNextThread=classesPerThread;
			if (i==numThreads-1)
				classesNextThread=numClasses-nextClass;
			rankers[i]=new RankerBySVM(nextClass, classesNextThread, data);
			nextClass+=classesPerThread;
			rankers[i].start();
		}
		
		attScoresByClass = new int[data.numClasses()][];
		
		for (int i = 0; i < numThreads; i++)
		{
			rankers[i].join();
			
			int firstClassRanker=rankers[i].getMinIndex();
			int[][] results=rankers[i].getResults();
			
			for (int j=0; j<results.length; j++)
				attScoresByClass[j+firstClassRanker]=results[j];
		}
	}
	else
	{
  ...
  private int[] rankBySVM(int classInd, Instances data) {
    ...
    try {
      ...
      int initial=trainCopy.numAttributes();
      ...
      while (numAttrLeft > 0) {
        ...
        System.out.println("Trained "+trainCopy.numAttributes()+"/"+initial);
  ...
  public String toString() {
    ...
    if (m_attScores == null) {
      ...
      text.append("\n\tUsing up to "+((getN_threads()==Integer.MAX_VALUE)?("all ("+Runtime.getRuntime().availableProcessors()+")"):getN_threads())+" threads if multiclass.");
    }
  ...
  public static void main(String[] args) {
    runEvaluator(new SVMThreadedAttributeEval(), args);
  }
}
