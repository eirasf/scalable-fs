...
/*
 *    ReliefFThreaded2AttributeEval.java
 ...
 * data.<br/>
 * <br/>
 * Multithread implementation.<br /><br />
 ...
 * @version $Revision: 1 $
 */
public class ReliefFThreadedAttributeEval extends ASEvaluation implements
  AttributeEvaluator, OptionHandler, TechnicalInformationHandler {
	
	private class HitMissFinder extends Thread
	{
		...
		public double[][][] getNeighbors(int i)
		{
			return m_karray[i];
		}

		private int[] instNums;
		
		public int[] getInstNums()
		{
			return instNums;
		}

		public HitMissFinder(int[] instNums)
		{
			m_worst = new double[m_numClasses]; //Aux - Worst distance value of the kknum neighbors for each class. Used to check whether a new value should be added to the nearest neighbors.
			m_index = new int[m_numClasses]; //Index of the neighbor with the worst distance.
			m_stored = new int[instNums.length][m_numClasses]; //Number of neighbors stored.
			// num classes (1 for numeric class) knn neighbours,
			// and 0 = distance, 1 = instance index
			m_karray = new double[instNums.length][m_numClasses][m_Knn][2];

			this.instNums=instNums;
		}
		
		/**
		 * Find the K nearest instances to supplied instance if the class is
		 * numeric, or the K nearest Hits (same class) and Misses (K from each
		 * of the other classes) if the class is discrete.
		 * 
		 * @param instNums the index of the instance to find nearest neighbours of
		 */
		private void findKHitMiss()
		{
			for (int inst=0; inst<instNums.length; inst++)
			{
				if (instNums[inst]>=0)
				{
					//Reset m_index - No need to reset m_worst
					...
					Instance thisInst = m_trainInstances.instance(instNums[inst]);
					...
					for (i = 0; i < m_numInstances; i++)
					{
						if (i != instNums[inst])
						{
							...
							// add this diff to the list for the class of this
							// instance
							if (m_stored[inst][cl] < m_Knn)
							{
								m_karray[inst][cl][m_stored[inst][cl]][0] = temp_diff;
								m_karray[inst][cl][m_stored[inst][cl]][1] = i;
								m_stored[inst][cl]++;
		
								// note the worst diff for this class
								for (j = 0, ww = -1.0; j < m_stored[inst][cl]; j++)
									if (m_karray[inst][cl][j][0] > ww)
									{
										ww = m_karray[inst][cl][j][0];
										m_index[cl] = j;
									}
								...
								/*
								 * if we already have stored knn for this class then
								 * check to see if this instance is better than the
								 * worst
								 */
							{
								if (temp_diff < m_karray[inst][cl][m_index[cl]][0])
								{
									m_karray[inst][cl][m_index[cl]][0] = temp_diff;
									m_karray[inst][cl][m_index[cl]][1] = i;
   ...
   * Constructor
   */
  public ReliefFThreadedAttributeEval() {
    resetOptions();
  }
...
/**
  ...
  public String[] getOptions() {
    ...
    options.add("-P");
    options.add("" + ((getN_threads()==Integer.MAX_VALUE)?0:getN_threads()));
  ...
  public void buildEvaluator(Instances data) throws Exception {
    ...
    //When the user chooses that the neighbours are weighted according to their distance, we specify a weight for each position (1st neighbour, 2nd neighbor....)
    ...
    //Computes the probability of each class as (# of instances of that class/total # of instances)
    if (!m_numericClass) {
      ...
      //Compute probabilities by dividing by the total number of instances
      for (int i = 0; i < m_numClasses; i++) {
        m_classProbs[i] /= m_numInstances;
      }
    }
    
    m_minArray = new double[m_numAttribs]; //Minimum value for each attribute. Used for normalization of values.
    m_maxArray = new double[m_numAttribs]; //Maximum value for each attribute. Used for normalization of values.
    ...
    //Loop through all instances to find min and max values for each attribute
    for (int i = 0; i < m_numInstances; i++) {
    ...
    if (numThreads>totalInstances)
    	numThreads=totalInstances;
    HitMissFinder[] workers=new HitMissFinder[numThreads];
    
    int instancesPerThread=totalInstances/numThreads;
    int counter=0;
    int[] instanceIndexesForThread=null;
    int numWorker=0;
    
    // process each instance, updating attribute weights
    for (int i = 0; i < totalInstances; i++)
    {
    	if ((instanceIndexesForThread==null) || (counter>=instanceIndexesForThread.length))
    	{
    		counter=0;
    		int newSize=instancesPerThread;
    		if (numWorker>=workers.length-1)
    			newSize=totalInstances-i;
    		
    		instanceIndexesForThread=new int[newSize]; //Created every time so that it can be passed to the workers.
    	}
    	
    	if (totalInstances == m_numInstances)
    		z = i; //Not random when all training instances used.
    	else
    	...
    	if (!(m_trainInstances.instance(z).isMissing(m_classIndex)))
    		instanceIndexesForThread[counter++]=z;
    	else
    		instanceIndexesForThread[counter++]=-1;
    	
    	if (counter>=instanceIndexesForThread.length)
    	{
	    	//No need to synchronize if z has the same value of one of the previous workers because all computations are performed with local variables.
			workers[numWorker]=new HitMissFinder(instanceIndexesForThread);
			workers[numWorker++].start();
    	}
    }
    ...
    for(int i = 0; i < workers.length; i++)
    {
		...
		int[] instNums=workers[i].getInstNums();
    	for (int j=0; j<instNums.length; j++)
    		if (instNums[j]>=0)
    		{
	    	    if (m_numericClass)
	    	    	updateWeightsNumericClass(instNums[j], workers[i].getNumNeighbors(j), workers[i].getNeighbors(j));
	    	    else
	    	    	updateWeightsDiscreteClass(instNums[j], workers[i].getNumNeighbors(j), workers[i].getNeighbors(j));
    		}
    	workers[i]=null;
    }
...
if (index==4)
   ...
   * @param neighborsPerClass
   * @param closestNeighbors
   */
  private void updateWeightsNumericClass(int instNum, int[] neighborsPerClass, double[][][] closestNeighbors) {
    int i, j;
    double temp, temp2;
    int[] tempSorted = null;
    ...
    if (m_weightByDistance) {
      tempDist = new double[neighborsPerClass[0]];

      for (j = 0, distNorm = 0; j < neighborsPerClass[0]; j++) {
        // copy the distances
        tempDist[j] = closestNeighbors[0][j][0];
        // sum normalizer
    ...
    for (i = 0; i < neighborsPerClass[0]; i++) {
      // P diff prediction (class) given nearest instances
      if (m_weightByDistance) {
        temp = difference(
          m_classIndex,
          inst.value(m_classIndex),
          m_trainInstances.instance((int) closestNeighbors[0][tempSorted[i]][1]).value(
            m_classIndex));
        temp *= (m_weightsByRank[i] / distNorm);
      } else {
        temp = difference(m_classIndex, inst.value(m_classIndex),
          m_trainInstances.instance((int) closestNeighbors[0][i][1])
            .value(m_classIndex));
        temp *= (1.0 / neighborsPerClass[0]); // equal influence
      }
      ...
      cmp = (m_weightByDistance) ? m_trainInstances
        .instance((int) closestNeighbors[0][tempSorted[i]][1]) : m_trainInstances
        .instance((int) closestNeighbors[0][i][1]);
      ...
      for (int p1 = 0, p2 = 0; p1 < inst.numValues() || p2 < cmp.numValues();) {
        ...
        if (m_weightByDistance) {
          ...
          temp2 *= (1.0 / neighborsPerClass[0]); // equal influence
        }
   ...
   * @param closestNeighbors
   */
  private void updateWeightsDiscreteClass(int instNum, int[] neighborsPerClass, double[][][] closestNeighbors)
  {
    int i, j, k;
    int cl;
    ...
    if (m_weightByDistance) {
      // do class (hits) first
      // sort the distances
      tempDistClass = new double[neighborsPerClass[cl]];

      for (j = 0, distNormClass = 0; j < neighborsPerClass[cl]; j++) {
        // copy the distances
        tempDistClass[j] = closestNeighbors[cl][j][0];
        // sum normalizer
      ...
      for (k = 0; k < m_numClasses; k++) {
        ...
        {
          // sort the distances
          tempDistAtt = new double[neighborsPerClass[k]];

          for (j = 0, distNormAtt[k] = 0; j < neighborsPerClass[k]; j++) {
            // copy the distances
            tempDistAtt[j] = closestNeighbors[k][j][0];
            // sum normalizer
    ...
    for (j = 0, temp_diff = 0.0; j < neighborsPerClass[cl]; j++) {
      Instance cmp;
      cmp = (m_weightByDistance) ? m_trainInstances
        .instance((int) closestNeighbors[cl][tempSortedClass[j]][1]) : m_trainInstances
        .instance((int) closestNeighbors[cl][j][1]);
      ...
      for (int p1 = 0, p2 = 0; p1 < inst.numValues() || p2 < cmp.numValues();) {
        ...
        if (m_weightByDistance) {
          ...
          if (neighborsPerClass[cl] > 0) {
            temp_diff /= neighborsPerClass[cl];
          }
        }
    ...
    for (k = 0; k < m_numClasses; k++) {
      if (k != cl) // already done cl
      {
        for (j = 0; j < neighborsPerClass[k]; j++) {
          Instance cmp;
          cmp = (m_weightByDistance) ? m_trainInstances
            .instance((int) closestNeighbors[k][tempSortedAtt[k][j]][1])
            : m_trainInstances.instance((int) closestNeighbors[k][j][1]);
          ...
          for (int p1 = 0, p2 = 0; p1 < inst.numValues()
            ...
            if (m_weightByDistance) {
              ...
              if (neighborsPerClass[k] > 0) {
                temp_diff /= neighborsPerClass[k];
              }
            }
            if (m_numClasses > 2) {
  ...
  public static void main(String[] args) {
    runEvaluator(new ReliefFThreadedAttributeEval(), args);
  }
}
