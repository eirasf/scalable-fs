 ...
 * @version $Revision: 1 $
 */
public class DiscretizeThreaded extends Filter implements SupervisedFilter,
  OptionHandler, WeightedInstancesHandler, TechnicalInformationHandler {
	...
	private class Converter extends Thread
	{
		...
		public Converter(int firstIndex, int numInstances, Instances data)
		{
		...
		public void run()
		{
			...
			{
				Instance instance = data.instance(firstIndex + k);
				int index = 0;
				...
				inst.setDataset(getOutputFormat());
				pushLocalInstance(inst);
				
				//Free memory space by nullifying the input instance.
				data.nullifyInstance(firstIndex + k);
			}
		}		

		private void pushLocalInstance(Instance instance)
		{
			...
			if (instance != null)
			{
				...
				{
					instance = (Instance) instance.copy(); // Don't modify
															// incoming
					// instance!
	...
	private class CutPointCalculator extends Thread
	{
		private int firstAttribute;
		private int numAttributes;
		private Instances data;
		
		public CutPointCalculator(int firstAttribute, int numAttributes, Instances data)
		{
			this.firstAttribute=firstAttribute;
			this.numAttributes=numAttributes;
			this.data=new Instances(data); //Use copy to preserve order
		}
		
		public void run()
		{
			int current=firstAttribute;
			for (int k = 0; k < numAttributes; k++)
			{
				current=firstAttribute+k;
				if ((m_DiscretizeCols.isInRange(current)) && (getInputFormat().attribute(current).isNumeric()))
					calculateCutPointsByMDL(current, data);
			}
		}
	}
  ...
  public boolean batchFinished() throws InterruptedException {
    ...
    if (m_CutPoints == null) {
      ...
      // If we implement saving cutfiles, save the cuts here
		...
		Converter[] converters=new Converter[numThreads];

		int totalInstances=getInputFormat().numInstances();
	    ...
	    {
	    	if (i==numThreads-1)
	    		instancesPerThread=totalInstances-counter;
	    	converters[i]=new Converter(counter, instancesPerThread, getInputFormat());
	    	converters[i].start();
	    	counter+=instancesPerThread;
	    }
      // Convert pending input instances
      //for (int i = 0; i < getInputFormat().numInstances(); i++) {
      //  convertInstance(getInputFormat().instance(i));
      //}
      /*while(getInputFormat().numInstances()>0)
      {
    	  convertInstance(getInputFormat().instance(0));
    	  getInputFormat().delete(0);
      }
      */
		...
		{
	    	converters[i].join();
	    	queues.pushAll(converters[i].getOutputQueue());
		}
	    this.setQueue(queues);
...
/*    for (int i = getInputFormat().numAttributes() - 1; i >= 0; i--) {
	...
	if (numThreads>n_threads)
		numThreads=n_threads;
	CutPointCalculator[] calculators=new CutPointCalculator[numThreads];

	int totalAttributes=getInputFormat().numAttributes();
	int attributesPerThread=totalAttributes/numThreads;
    int counter=0;
    ...
    {
    	if (i==numThreads-1)
    		attributesPerThread=totalAttributes-counter;
    	calculators[i]=new CutPointCalculator(counter, attributesPerThread, getInputFormat());
    	calculators[i].start();
    	counter+=attributesPerThread;
    }
    for (int i = 0; i < numThreads; i++)
	{
    	calculators[i].join();
  ...
  public static void main(String[] argv) {
    runFilter(new DiscretizeThreaded(), argv);
  }
}